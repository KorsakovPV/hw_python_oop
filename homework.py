...
class Calculator:
    ...
    def get_today_stats(self):
        ...
        spend_today = sum([i.amount if i.date==today else 0 for i in self.records])
        return spend_today

    def get_week_stats(self):
        end_week = dt.datetime.now().date()
        start_week = end_week - dt.timedelta(days = 6)
        spend_week = sum([i.amount if i.date<=end_week and i.date>=start_week else 0 for i in self.records])
        return spend_week            
...
class CashCalculator(Calculator):
    ...
    def get_today_cash_remained(self, currency): 
        ...
        if spend_today==self.limit:
            ...
            answer={'rub':['На сегодня осталось', 1, 'руб'],
                    'usd':['На сегодня осталось', self.USD_RATE, 'USD'],
                    'eur':['На сегодня осталось', self.EURO_RATE, 'Euro']}
            remains = self.limit-spend_today
            if currency=='rub' or currency=='usd':
                return '{0} {1:.1f} {2}'.format(answer[currency][0], remains/answer[currency][1], answer[currency][2]) #Тестирование проходит только с 1 знаком после запятой для rub и usd
            elif currency=='eur':
                return '{0} {1:.2f} {2}'.format(answer[currency][0], remains/answer[currency][1], answer[currency][2])
        elif spend_today>self.limit:
            answer={'rub':['Денег нет, держись: твой долг -', 1, 'руб'],
                    'usd':['Денег нет, держись: твой долг -', self.USD_RATE, 'USD'],
                    'eur':['Денег нет, держись: твой долг -', self.EURO_RATE, 'Euro']}
            remains = abs(self.limit-spend_today)
            if currency=='rub' or currency=='usd':
                return '{0} {1:.1f} {2}'.format(answer[currency][0], remains/answer[currency][1], answer[currency][2])
            elif currency=='eur':
                return '{0} {1:.2f} {2}'.format(answer[currency][0], remains/answer[currency][1], answer[currency][2])
